...
local p = luatool:Fetch(current_time, level(HPLevelType.kHeight, 0), param("P-PA"))
local t = luatool:Fetch(current_time, level(HPLevelType.kHeight, 2), param("T-K"))
local dp = luatool:Fetch(current_time, level(HPLevelType.kHeight, 2), param("TD-K"))

local i = 0
local res = {}

for i=1, #t do
  --if (IsValid(p[i]) and IsValid(dp[i]) and IsValid(t[i])) then
    t[i] = t[i]-273.15
    dp[i] = dp[i]-273.15
    p[i] = p[i]/100.0
